...
public class FeatureTest {
	...
	public void hitCaptainArmor(){
		assertTrue(board.placeShip(new Ship("BATTLESHIP"), 1, 'B', false));
		var firstShot = board.attack(1, 'H');
		assertEquals(AtackStatus.MISS, firstShot.getResult());
		var secondShot = board.attack(1,'E');
		...
		firstShot = board.attack(4, 'A');
		assertEquals(AtackStatus.MISS, firstShot.getResult());
		//assertTrue(destroyer.armor == damaged);
		secondShot = board.attack(3,'A');
		assertEquals(AtackStatus.HIT, secondShot.getResult());
	}
